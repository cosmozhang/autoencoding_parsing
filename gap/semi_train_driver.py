...
from src import semi_ncrfae_parser
import pdb
...
if __name__ == '__main__':
    ...
    if options.max_len:
        sentencesData = [sent for sent in sentencesData if len(sent) <= int(options.max_len)]
    ...
    if uprop > 0.0:
        ...
        u_idxs = list(range(max_idx_l + 1, max_idx_u + 1))
    ...
    parser = semi_ncrfae_parser.MSTParserLSTM(words, p2i, rels, w2i, options)

    parser.init_decoder(labeledSentences)
